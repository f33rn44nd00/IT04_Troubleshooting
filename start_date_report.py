...
def main():
    ...
    #download the file into a structure
    file_url = "https://storage.googleapis.com/gwg-content/gic215/employees-with-date.csv"
    file_lines = get_file_lines(file_url)
    reader = csv.reader(file_lines[1:])
    lines = list(reader)
    lines.sort(key = lambda x: x[3])
    while start_date < datetime.datetime.today():
        start_date, employees = get_same_or_newer(start_date,lines)
